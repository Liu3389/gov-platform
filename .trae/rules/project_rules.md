...
| 前端页面/组件/API调用 | `.trae/rules/tasks/frontend.md` |
...
| 成员E（前端） | gov-frontend（Vue3 + Vite） | 5173 |
...
| 15 | 已有 SQL 文件修改 | → 新建 Flyway V版本号 ALTER 文件（db/schema/） |
...
| RabbitMQ | 127.0.0.1:**5673** / :**15673**（guest/guest） |

启动顺序：`bash start-all.sh --quick`（Mac/Linux）或 `powershell -File start-all.ps1 -Quick`（Windows）→ 启动网关 → 启动自己模块

### JDK 17 安装（强制要求）

项目已配置 `maven-enforcer-plugin`，使用 JDK 8 编译会直接报错并给出安装指引。

| 操作系统 | 安装方式 |
|---------|---------|
| **Windows** | 下载 [Eclipse Temurin JDK 17](https://adoptium.net/download/) 安装，设置 `JAVA_HOME` 环境变量 |
| **WSL2 (Ubuntu)** | `sudo apt update && sudo apt install openjdk-17-jdk -y` |
| **macOS** | `brew install openjdk@17` |
| **IntelliJ IDEA** | `File → Project Structure → SDK` 选择 JDK 17 |
...
| 新增 Entity 字段 | 对应的 DTO / VO → 建表 SQL（`db/schema/`） |
| 新增/修改 VO 字段 | 所有使用该 VO 的 Feign 调用方 → `gov-common` 编译验证 |
| 新增数据库表 | `db/schema/` Flyway V版本号 SQL → Entity → Mapper → Service |
| 修改数据库表字段 | `db/schema/` Flyway ALTER SQL → Entity 字段 → DTO/VO 同步 |
...
| 14 | 新表/SQL | `db/schema/` 有对应 Flyway SQL 文件（V版本号命名），种子数据在 `db/data/` |
...
```
【AI 上下文 — 你的用户是成员E（前端开发）】
负责模块：gov-frontend（Vue3 + Vite + TypeScript）
端口：5173（Vite dev server）
需要启动：前端 dev server（npm run dev）
你可以改：gov-frontend/ 下的所有文件
你不能改：所有后端模块（gov-gateway/、gov-*-service/、gov-common/）、docker/、scripts/
你的依赖：网关（8091）提供的 /api/v1/* REST API
你的职责：
  - 基于 Vue3 + Vite + TypeScript 开发前端页面
  - 使用 Axios 调用后端网关 API（统一封装在 src/api/）
  - 使用 Pinia 进行状态管理
  - 使用 Vue Router 进行路由管理
  - 组件库：Element Plus（推荐）或 Ant Design Vue
  - 所有 API 调用走网关 http://localhost:8091/api/v1/{服务}/{接口}
  - 登录后 Token 存 localStorage，每次请求带 Authorization: Bearer <token>
  - 页面开发前先去 Knife4j(http://localhost:8091/doc.html) 确认接口可用
前端项目初始化命令：
  npm create vite@latest gov-frontend -- --template vue-ts
  cd gov-frontend && npm install && npm install axios pinia vue-router element-plus
注意：前端不经过 Nacos，只通过网关访问后端。本地开发时 Vite proxy 可选。
```
...
| 队员新增了数据库表 | "新增表 {表名}，请在 PR 描述中说明，并确保 SQL 文件已提交到 db/schema/（Flyway V版本号）" |
...
| 成员E说"页面做好了" | "请在浏览器打开 http://localhost:5173 验证页面功能，并在 Network 面板确认 API 调用正常（200/401 分别对应登录/未登录）" |
| 成员E新增了 API 调用 | "新增 API 调用需确认接口已在 Knife4j 上线，并检查请求/响应格式与后端 VO/DTO 一致" |
| 队员让成员E改后端接口 | "成员E是前端开发，不能改后端代码。如需修改接口请联系对应后端成员" |
...
禁止：
  ...
  - 改非自己模块的文件
  - 改 gov-common（仅组长可改）
  - 改 db/schema/ 中已有 SQL 文件（新表用 Flyway V版本号，改表用新版本 ALTER）
```
