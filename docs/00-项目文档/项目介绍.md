...
graph LR
    subgraph Local[你的本地电脑]
        ...
        subgraph Entry[入口层]
            ...
            Gateway[Gateway 网关<br>localhost:8080]
        end
...
| gov-item-service      | 8082        | `gov_item` / db 2      |
...
> 组长负责 `gov-platform-parent`、`gov-common`、`gov-gateway` (8080)、`gov-monitor-service` (8090)、`gov-message-service` (8089协助)、Docker 环境、文档规范；并统筹代码 Review。

| 成员   | 负责模块                                                             | 主要职责                                     |
| ---- | ---------------------------------------------------------------- | ---------------------------------------- |
| 组员 A | `gov-user-service` (8081)`gov-item-service` (8082)               | 用户认证、登录注册、CA证书绑定、事项管理、办事指南               |
| 组员 B | `gov-reception-service` (8083)`gov-activiti-service` (8084)【核心】  | 统一受理、一窗登记、办件跟踪、Activiti 审批流转、待办/已办、会签/转办 |
| 组员 C | `gov-license-service` (8085)`gov-complaint-service` (8086)       | 电子证照生成/验真、OFD/PDF生成、投诉建议、督办催办            |
| 组员 D | `gov-open-service` (8087)`gov-datashare-service` (8088)`gov-web` | 政务公开、数据共享、Vue 前端、前后端联调、Nginx 本地代理        |
...
| 端口占用           | `lsof -i :3306` | `netstat -ano \| findstr 3306`     |
| IDEA 编码        | UTF-8           | UTF-8                              |
